...
import { customNetworkTimeoutThreshold, customUrlForIPCheck, DNSAPIDetails } from './configs/index.js'
import runTests from './test/index.js'
import tests from './test/tests.js'

console.log(customUrlForIPCheck, customNetworkTimeoutThreshold)
const NETWORK_TIMEOUT_THRESHOLD_MS = customNetworkTimeoutThreshold ? customNetworkTimeoutThreshold : 20000
const URL_FOR_IP_CHECK = customUrlForIPCheck ? customUrlForIPCheck : 'http://ipecho.net/plain'
const DELAY_BETWEEN_IP_CHECKS = 2000
  ...
  try {
    ...
    while(!issue) {
      try {
        ...
        .catch(err =>{ console.error(err); return new CheckIPError(err) })
        ...
        if(app.state.firstCheck) { 
          ...
          if(err instanceof CheckIPError) {
            console.error(`===> Error issuing network call. Delaying for ${DELAY_BETWEEN_IP_CHECKS / 1000} seconds, then reattempting network call.
`, err)
            await delay()
            continue
          } else {
